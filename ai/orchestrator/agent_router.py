...
def synthesizer_node(state: CopilotState) -> dict:
    ...
    if api_key:
        system_prompt = (
            ...
            "1. Health & Telemetry: Summarize the system health score, stale tables, or volume drift from observability.\n"
            "2. Error Diagnosis: If there is a failed execution run, read the error message and the detailed trace log. "
            "Diagnose the exact root cause of the crash (e.g. check for missing columns, database timeouts, credentials errors).\n"
            "3. SQL Fix Recommendation: For any diagnosed failures, write the exact, complete SQL queries or commands "
            "the engineer must run to resolve the error (enclosed in markdown code blocks).\n"
            "4. Webhook Alerts: If the user asked about alerts or webhooks, list active webhooks or summarize registry actions.\n"
            "5. Data Lineage Flowchart: If data lineage data (`lineage_flow` or `lineage_details`) is present in the JSON, "
            "you MUST construct and draw a beautiful Mermaid flowchart diagram (using ```mermaid ... ``` syntax, flowchart LR, "
            "with double brackets like `PipelineName[[\"PipelineName\"]]` for pipelines, and cylindrical shapes like `TableName[(\"TableName\")]` for tables) "
            "to visually depict the source-to-target dependency flow.\n\n"
            "Be specific - cite exact table names, pipeline names, row counts, and parameters from the JSON. "
            "Always end with a clear next-steps bullet list."
        )
        user_prompt = (
