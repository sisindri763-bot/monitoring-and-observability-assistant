...
class FreshnessRepository:
    ...
    def get_latest_source_updates(self):
        query = """
        SELECT 
            s1.table_name, 
            s1.last_updated, 
        ...
        GROUP BY s1.table_name, s1.last_updated, s1.collected_at
        """
        return self._execute_query(query)

    def get_latest_target_updates(self):
        query = """
        SELECT 
            t1.table_name, 
            t1.last_updated, 
        ...
        GROUP BY t1.table_name, t1.last_updated, t1.collected_at
        """
        return self._execute_query(query)

    def get_source_table_history(self, table_name):
        query = """
        SELECT DISTINCT last_updated, collected_at
        FROM SOURCE_DB_METADATA
        WHERE table_name = %s
